...
| `kernel/109-media-rkvdec-vdpu383-powerup-warmup` | fixes RK3576 hardware H.264 decode, which is otherwise wrong for a whole power cycle at a time. The corrupt rows are an H.264 normal-filter deblock applied to a horizontal edge the stream does not have, with the p side reading zero; the threshold it clips against is latched when the power domain comes up, and nothing programmed per run or allocated per context moves it. Decodes a canned 2x2 H.264 IDR frame through the link-mode engine from `runtime_resume` — the one point that precedes every decode of the power cycle — which is what the vendor driver's own RK3576-only workaround does. What the block needs is to *complete* a decode, so the job carries only the decode mode, a core timeout, the stream and the frame to write it into: enabling the link engine without submitting a job is worth nothing (129/400 against a control's 133/400, p = 0.82), and dropping the core timeout leaves a job that reports ready without decoding and does not fix anything (134/400, p = 0.22), while the RCB pointers, the error-processing word and the CABAC masks are each removable at 0/400. Measured against an interleaved control on the H96 MAX M9, taking the power-up as the trial: **0/400 against the control's 126/400**, CMH stratified in blocks of 100 rounds p = 1e-33, HEVC bit-exact in every arm. The vendor-shaped job it grew out of read 0/173 against 40/173 over five campaigns at 1080p and 4K (CMH p = 6e-11) plus 0/200 on a soak. The job as it stands then soaked 200 further power-ups alternating 1080p and 4K on a kernel carrying it unconditionally, none corrupt and no warm-up warning; what it has not had is the 8.5 hour sleep test. Silent when it works; warns on a warm-up timeout or a non-zero error status — and that warning matters, because a job the block never completes leaves it unable to decode either codec for the rest of the power cycle. |
