...
| `kernel/103-asoc-rockchip-sai-reapply-routing-on-resume` | makes `rockchip,sai-tx-route` take effect at all. The SAI driver programs `SAI_PATH_SEL` once from probe and the value does not survive to the running device, so the register reads its reset mapping whatever the property says — a board whose codec sits on an SDO port the reset mapping does not drive gets a card that streams correctly into silence. Re-applies the routing from `runtime_resume`, which also keeps it across PM cycles. Every in-tree board describes the identity mapping the register already holds, so none of them can observe the bug. |
| `kernel/104-media-rkvdec-vdpu383-read-cache` | arms the VDPU383's three reference read caches. The vendor driver configures, invalidates and bounds them before every decode; the mainline HEVC and H.264 backends issue no cache configuration at all, and cannot — probe maps only the `"function"` and `"link"` reg regions, so the window those registers live in is never mapped, though `rk3576.dtsi` has described it (`"cache"`, 0x27b00600) all along. The caches serve reads only, so decoded output is unchanged; a `read_cache` module parameter measures the two configurations against each other without a rebuild. |
| `kernel/108-media-rkvdec-guard-null-context-in-irq-handler` | stops `rkvdec_irq_handler()` dereferencing a NULL context. It resolves the current job with `v4l2_m2m_get_curr_priv()` and hands the result straight to a variant handler that opens with `ctx->dev`, and that function returns NULL when no job is current. Nothing in the decode path can reach it, but 109 submits work of its own with no context behind it, so this is 109's prerequisite as well as a robustness fix in its own right. Written to stand alone, with no erratum context, so it can be offered upstream separately. |
| `kernel/109-media-rkvdec-vdpu383-powerup-warmup` | fixes RK3576 hardware H.264 decode, which is otherwise wrong for a whole power cycle at a time. The corrupt rows are an H.264 normal-filter deblock applied to a horizontal edge the stream does not have, with the p side reading zero; the threshold it clips against is latched when the power domain comes up, and nothing programmed per run or allocated per context moves it. Decodes a canned 2x2 H.264 IDR frame from `runtime_resume` — the one point that precedes every decode of the power cycle — which is what the vendor driver's own RK3576-only workaround does, reaching the block the way a codec backend runs a decode: the register image written into the function window, `DEC_ENABLE` kicked, the completion polled. What the block needs is to *complete* a decode, so the job carries only the decode mode, a core timeout, the stream and the frame to write it into: enabling the link engine without submitting a job is worth nothing (129/400 against a control's 133/400, p = 0.82), and dropping the core timeout leaves a job that reports ready without decoding and does not fix anything (134/400, p = 0.22), while the RCB pointers, the error-processing word and the CABAC masks are each removable at 0/400. How the job reaches the block makes no difference — the vendor submits the same job through the link-mode engine, which as a repair arm cleared 33 of 33 latched power-ups against the direct kick's 36 of 36 — so it carries neither the link node nor the link-mode control registers. Measured against an interleaved control on the H96 MAX M9, taking the power-up as the trial: **0/400 against the control's 128/400**, CMH stratified in blocks of rounds p = 1e-34, HEVC bit-exact in every arm; the same job through link mode measured 0/400 against 126/400. The vendor-shaped job it grew out of read 0/173 against 40/173 over five campaigns at 1080p and 4K (CMH p = 6e-11) plus 0/200 on a soak. The job as it stands then soaked 200 further power-ups alternating 1080p and 4K on a kernel carrying it unconditionally, none corrupt and no warm-up warning; what it has not had is the 8.5 hour sleep test. Silent when it works; warns on a warm-up timeout or a non-zero error status — and that warning matters, because a job the block never completes leaves it unable to decode either codec for the rest of the power cycle. |
