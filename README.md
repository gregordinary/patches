...
| Dir | Scope |
|-----|-------|
| [`rocket/`](rocket/) | Rockchip NPU — patches, UAPI header, and notes for the mainline `rocket` DRM accel driver (RK3588). Migrated from the standalone `rocket-patches` repo with history preserved. |
| [`media-accel/`](media-accel/) | Rockchip HW video transcode (RK35xx/RK3588) — kernel decode/encode/RGA, ffmpeg-rockchip, and MPP/RGA userspace patches. Copied from `debian-rk1` (canonical home; the build repo keeps its own copies). |
...
does not apply. Offline builds fetch the pinned revision manually. The frozen
`debian-rk1` build repo keeps its own copies of the patches it needs.
...
- **`rocket/` scope** — the NPU driver patches and `perf-probe-v2-safe.c` are
